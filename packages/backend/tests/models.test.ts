...
describe('Database Models', () => {
  ...
  beforeEach(async () => {
    vi.clearAllMocks()
    const { pool } = await import('../src/db/init.js')
    mockPool = pool
  })
  ...
  describe('User Models', () => {
    ...
    it('should return undefined when user by email not found', async () => {
      ...
      expect(mockPool.query).toHaveBeenCalledWith('SELECT * FROM users WHERE email = $1', ['nonexistent@example.com'])
  ...
  describe('Task Models', () => {
    ...
    it('should update a task with all fields', async () => {
      ...
      expect(mockPool.query).toHaveBeenCalledWith(
        expect.stringContaining('UPDATE tasks SET title = $1, description = $2, completed = $3, updated_at = CURRENT_TIMESTAMP WHERE id = $4 AND user_id = $5 RETURNING *'),
    ...
    it('should update a task with partial fields', async () => {
      ...
      expect(mockPool.query).toHaveBeenCalledWith(
        expect.stringContaining('UPDATE tasks SET title = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2 AND user_id = $3 RETURNING *'),
        ['Updated Title', 1, 1]
      )
      expect(result).toEqual(mockTask)
    })
